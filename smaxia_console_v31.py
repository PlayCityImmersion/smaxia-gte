# =============================================================================
# SMAXIA GTE Console V31.10.22 — ISO-PROD (FICHIER UNIQUE)
# =============================================================================
# OBJECTIF V31.10.22 (KERNEL V10.6.1) :
...
def _looks_like_question(seg: str, intent_verbs: List[str], keep_cfg: Dict[str, Any]) -> bool:
    ...
    # Keep more aggressively to avoid under-extraction
...
def _looks_like_answer(seg: str, keep_cfg: Dict[str, Any]) -> bool:
    ...
    if L < int(keep_cfg.get("min_len", 30)) or L > int(keep_cfg.get("max_len", 2600)):
        return False
    # answers often contain math and reasoning words; keep broad
    digits = len(re.findall(r"\d", seg))
...
def split_into_a_segments(text: str, pack: Dict[str, Any]) -> Tuple[List[str], Dict[str, Any]]:
    ...
    # answers: cut by paragraphs + numbered headings
    regs = _compile_any([
        r"(?m)^\s*(\d{1,2}|[a-h])\s*[\)\.\-:]\s+",
...
def align_q_to_a(qs: List[str], ans: List[str], pack: Dict[str, Any]) -> Tuple[List[Optional[int]], Dict[str, Any]]:
    ...
    # greedy deterministic: iterate q in order, choose best unused answer
...
def generate_frt(primary_op: str, triggers: List[str], ari: AriTrace, pack: Dict[str, Any]) -> FRT:
    ...
    # enforce required keys (pack-driven list)
    for k in required:
...
def sigma_similarity(qc_a: QCItem, qc_b: QCItem) -> float:
    # σ : Jaccard on (all_ops ∪ triggers) — deterministic, invariant, no embeddings
...
def t_rec_from_exam_date(exam_date: Optional[str], run_ts: str, t_rec_min: float) -> float:
    # t_rec is recency factor, >= t_rec_min, deterministic relative to run_ts
    try:
...
def progressive_select_per_chapter(qcs: List[QCItem], N_total: int, pack: Dict[str, Any], top_k: int) -> List[QCItem]:
    ...
    # choose only QC with at least 1 posable qi
    ...
    for _ in range(min(top_k, len(remaining))):
        ...
        best_score = -1.0
        for qc in remaining:
            # t_rec already set on QC (computed from cluster items)
...
def run_pipeline(pack: Dict[str, Any], library: List[Dict[str, Any]], volume: int, threads: int) -> Dict[str, Any]:
    ...
    # fixed run timestamp for determinism
    ...
    def process_pair(pair: Dict[str, Any]) -> Tuple[List[QiItem], Optional[QuarantineEntry], Dict[str, Any]]:
        ...
        if len(qs) < 6:
            # do not quarantine entire pair, but flag low atomization
        ...
        for i, qtxt in enumerate(qs):
            ...
            # add low extraction / low alignment RCs on the item when relevant
    ...
    # per chapter bucket counts (anti-singleton logic)
    ...
    for (cc, primary_op), items in sorted(buckets.items(), key=lambda x: (x[0][0], x[0][1])):
        ...
        # Kernel anti-singleton: skip only if singleton, not only, and no posable
        ...
        # FRT (per operator) — use qc triggers + ari of "representative" (highest conf)
        rep = max(items, key=lambda z: float((z.ari or {}).get("confidence_global", 0.0)))
        frt = generate_frt(primary_op, qc_triggers, AriTrace(**rep.ari), pack)
        frt_pack[frt.frt_id] = asdict(frt)

        # delta_c from chapter taxonomy (pack-driven)
        ...
        # t_rec : from cluster exam dates (use most recent date if available)
        dates = [x.source_exam_date for x in items if x.source_exam_date]
        exam_date = max(dates) if dates else None
        t_rec = t_rec_from_exam_date(exam_date, run_ts, t_rec_min)
    ...
    # normalize Psi_q per chapter (Kernel: Ψq in [0,1] within chapter)
    ...
    # assign qc_id to qi items and orphan detection
    ...
    for qi in qi_items:
        if qi.qi_id in mapping:
            ...
            qi.qc_id = None
            qi.is_orphan = True
            # orphan only counts for POSABLE Qi
            if qi.has_rqi and qi.posable.get("is_posable"):
                orphans += 1

    # IA2 checks + evidence per QC
    ia2_ok = True
    ia2_issues = []
    for qc in qc_pack:
    ...
    sanity_max = float(params.get("seal_sanity_dup_ratio_max", 0.35))
    sanity_ok = dup <= sanity_max

    # Extraction KPI gating
    kpi = extraction_kpi(qi_items, processed_pairs)
    min_avg = float(params.get("seal_min_avg_qi_per_pair", 8.0))
    min_total = float(params.get("seal_min_total_qi", 160))
    extraction_ok = (kpi["avg_qi_per_pair"] >= min_avg) and (kpi["qi_total"] >= min_total)

    # Build evidence pack (proof artifact)
    evidence_pack = {
        "version": APP_VERSION,
        "timestamp": run_ts,
        "pack_id": pack.get("pack_id"),
...
def seal_reason(audit: Dict[str, Any], pack: Dict[str, Any], new_qc_count: int) -> str:
    params = pack_get(pack, "f1_f2_params", {}) or {}
    cov_th = float(params.get("seal_coverage_threshold", 0.80))
...
def run_saturation(pack: Dict[str, Any], library: List[Dict[str, Any]], vol_start: int, vol_max: int, step: int, threads: int, top_k: int) -> Dict[str, Any]:
    ...
    for vol in range(vol_start, vol_max + 1, max(1, step)):
        ...
        # stop condition: new_QC == 0 (Kernel saturation)
        if new_qc_count == 0:
            break

    assert final is not None

    # Selection F2 per chapter on final qc_pack
    ...
    # SEALED decision (strict Kernel + anti-faux SEALED)
    ...
    # run_signature (SHA256 over all outputs)
    signature_payload = {
        "version": APP_VERSION,
    ...
    with st.sidebar:
        ...
        if st.button("🚀 ACTIVER", use_container_width=True, type="primary"):
            try:
                ...
                # set default subject/level from taxonomy
        ...
        if not lib:
            ...
            if st.button("🚀 LANCER TEST SATURATION", use_container_width=True, type="primary"):
                with st.spinner("Pipeline en cours..."):
                    try:
                        st.session_state.logs = st.session_state.logs[-4000:]  # keep bounded
