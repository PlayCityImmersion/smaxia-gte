#!/usr/bin/env python3
"""SMAXIA GTE CORE v14.3.4 — ISO-PROD — GPT Instructions Aligned
Pipeline: DA0→AR0→CAP→VSP→CEP→DA1→Atoms→QC→FRT→ARI→F1/F2→Triggers
F1/F2: TEST_CLEAR mode (R&D). No A2 pack pretense.
Zero random. Zero simulation. Zero hardcode pays."""
...
def _invariant_queries(cn):
    return[f"{cn} Ministry of Education official website",f"{cn} Department of Education official website",
           f"{cn} national examinations council official",f"{cn} examination board official",
           f"{cn} official syllabus curriculum",f"{cn} past papers official PDF"]
def _authority_score(url,html,aux=""):
    score,reasons=0.0,[]
    dom=urlparse(url).netloc.lower();text=(html or"").lower();combined=text+" "+(aux or"").lower()
    ...
    org=[("ministry of education","OFFICIAL_EDU"),("department of education","OFFICIAL_EDU"),
         ("ministère de l'éducation","OFFICIAL_EDU"),("ministère de l'enseignement","OFFICIAL_EDU"),
         ("ministerio de educación","OFFICIAL_EDU"),("ministério da educação","OFFICIAL_EDU"),
         ("national examinations","OFFICIAL_EXAM"),("examinations council","OFFICIAL_EXAM"),
         ("exam board","OFFICIAL_EXAM"),("government of","GOV_SELF"),("official website","OFFICIAL_SELF")]
    for token,tag in org:
        if token in combined:score=max(score,0.75);reasons.append(tag);break
    for token in["syllabus","curriculum","examination","past papers","baccalauréat","programme scolaire"]:
        if token in combined:score=min(score+0.05,1.0);reasons.append(f"EDU:{token[:15]}");break
    for token in["legal notice","mentions légales","contact","about us"]:
        if token in combined:score=min(score+0.03,1.0);reasons.append("HAS_LEGAL");break
...
def _search_api(cn,queries,diag):
    ...
    return cands
def _wikidata_ar0(cn,diag):
    try:
        import urllib.request,urllib.parse
        for term in[f"Ministry of Education ({cn})",f"Ministry of Education {cn}",f"Education in {cn}"]:
            su='https://en.wikipedia.org/w/api.php?'+urllib.parse.urlencode({'action':'query','list':'search','srsearch':term,'format':'json','srlimit':3})
            req=urllib.request.Request(su,headers={'User-Agent':'SMAXIA-GTE/14.3.4'})
            ...
            for res in data.get('query',{}).get('search',[]):
                ...
                req2=urllib.request.Request(wu,headers={'User-Agent':'SMAXIA-GTE/14.3.4'})
                ...
                for pid,page in data2.get('query',{}).get('pages',{}).items():
                    ...
                    req3=urllib.request.Request(wdu,headers={'User-Agent':'SMAXIA-GTE/14.3.4'})
                    ...
                    official=[u for u in urls if any(k in u.lower() for k in['.gov','.gouv','.edu','.ac.'])]
                    if official:
                        for u in official:diag.append({"p":"wikidata","url":u,"qid":qid})
                        return[{"url":u,"title":title,"provider":"wikidata","qid":qid} for u in official]
    except:pass
    return[]
def da0_discover_ar0(iso2):
    log_ev("DA0_AR0",iso2);db,_=_cdb();cn=db.get(iso2,iso2)
    queries=_invariant_queries(cn);diag=[]
    cands=_search_api(cn,queries,diag)+_wikidata_ar0(cn,diag)
    if not cands:
    ...
    for c in cands:
        ...
        if sc not in range(200,300):
            # PROOF_B: indirect authority via annuaire + TLD
            ...
            elif any(k in dom for k in[".edu",".ac."]):pb_score=max(pb_score,0.60);pb_reasons+=["TLD_EDU_INDIRECT","PROOF_B"]
            verified.append({"url":u,"domain":dom,"http_status":sc,"score":pb_score,"reasons":pb_reasons,"accessible":False,"provider":c.get("provider",""),"proof":"B_INDIRECT"})
            continue
        auth=_authority_score(u,html)
        verified.append({"url":u,"domain":dom,"http_status":sc,"score":auth["score"],"reasons":auth["reasons"],"accessible":True,"provider":c.get("provider",""),"title":c.get("title","")[:80],"proof":"A_DIRECT"})
    verified.sort(key=lambda x:(-x["score"],sha(x["url"])))
    ar0_edu=next((v for v in verified if "OFFICIAL_EDU" in v.get("reasons",[]) and v["score"]>=0.55),None)
    ar0_exam=next((v for v in verified if "OFFICIAL_EXAM" in v.get("reasons",[]) and v["score"]>=0.55),None)
    if not ar0_edu and not ar0_exam:
        best=next((v for v in verified if v["score"]>=0.55),None)
        if best:ar0_edu=best
    selected={"AR0_EDU":ar0_edu,"AR0_EXAM":ar0_exam}
    domains=[v["domain"] for v in[ar0_edu,ar0_exam] if v]
    if not domains:
    ...
    pack={"status":"OK","iso2":iso2,"country":cn,"queries":queries,"candidates":verified[:50],"selected":selected,"domains":list(set(domains)),"diag":diag}
    pack["sha256"]=sha(cj(sv(pack)));log_ev("AR0_OK",str(domains))
    return pack
# ═══ B2: CAP — FROM AR0 ONLY ═══
def cap_build(ar0,iso2):
    ...
    for url in seeds:
        ...
        for link in _links(html,url,lambda u,t:urlparse(u).netloc in allowed)[:30]:
            lu,lt=link["url"],link["text"].lower()
            if lu in visited:continue
            if ".pdf" in lu.lower():
                pdfs.append({"url":lu,"text":link["text"][:80],"domain":urlparse(lu).netloc,"source_page":url})
            elif any(k in lt or k in lu.lower() for k in["exam","syllabus","programme","curriculum","past paper","sujet","annale","bac","result","archive"]) and len(visited)<40:
                visited.add(lu)
                h2,s2,_,_=http_get(lu,timeout=12)
                if s2 in range(200,300):
                    pages.append({"url":lu,"domain":urlparse(lu).netloc,"status":s2,"role":_page_role(h2),"length":len(h2 or"")})
                    for pl in _links(h2,lu,lambda u,t:".pdf" in u.lower() and urlparse(u).netloc in allowed)[:20]:
                        pdfs.append({"url":pl["url"],"text":pl["text"][:80],"domain":urlparse(pl["url"]).netloc,"source_page":lu})
...
def _page_role(html):
    ...
    if any(k in low for k in["past paper","annale","sujet","archive","previous exam"]):return"EXAM_ARCHIVE"
...
def _detect_grading(pages):
    for p in pages:
        ...
        if "/100" in low or "percent" in low:return{"max":100,"pass_threshold":50,"description":"Scale 0-100","source":"ar0_crawl"}
        if "gpa" in low or "4.0" in low:return{"max":4,"pass_threshold":2,"description":"GPA 0-4.0","source":"ar0_crawl"}
    return{"max":0,"pass_threshold":0,"description":"[NOT_DETECTED]","source":"NONE"}
# ═══ B3: VSP ═══
def vsp_validate(cap):
    ...
    C("has_exam_or_syllabus",n_exam>=1 or any(p.get("role") in["SYLLABUS","EXAM_ARCHIVE"] for p in cap.get("authority_graph",[])))
...
def da0_find_pdfs(ar0,cap):
    log_ev("DA0_PDF")
    manifest,diag=[],[];cn=cap.get("country_name","");domains=cap.get("authority_domains",[])
    for d in cap.get("documents",[]):
        manifest.append({"url":d["url"],"type":_clf_pdf(d["url"],d.get("text","")),"hash":sha(d["url"])[:32],"domain":d.get("domain",""),"text":d.get("text","")[:80],"provider":"ar0_crawl","auth_score":0.9})
    # Search API: site:domain PDF
    key=os.environ.get("SERPAPI_KEY")
    if key and len(manifest)<5:
        try:
            import requests as rq
            for dom in domains[:2]:
                for q in[f"site:{dom} past papers PDF",f"site:{dom} sujet corrige PDF"]:
                    r=rq.get("https://serpapi.com/search",params={"q":q,"api_key":key,"num":10},timeout=15)
                    diag.append({"p":"serpapi","q":q[:50],"s":r.status_code})
                    if r.status_code==200:
                        for res in r.json().get("organic_results",[]):
                            u=res.get("link","")
                            if".pdf" in u.lower():manifest.append({"url":u,"type":_clf_pdf(u,res.get("title","")),"hash":sha(u)[:32],"domain":urlparse(u).netloc,"text":res.get("title","")[:80],"provider":"serpapi","auth_score":0.9})
        except:pass
    gk,gcx=os.environ.get("GOOGLE_CSE_KEY"),os.environ.get("GOOGLE_CSE_CX")
    if gk and gcx and len(manifest)<3:
        try:
            import requests as rq
            for q in[f"{cn} exam past papers PDF",f"{cn} sujet corrige PDF"]:
                r=rq.get("https://www.googleapis.com/customsearch/v1",params={"q":q,"key":gk,"cx":gcx},timeout=15)
                diag.append({"p":"gcse","q":q[:50],"s":r.status_code})
                if r.status_code==200:
                    for it in r.json().get("items",[]):
                        u=it.get("link","")
                        if".pdf" in u.lower():manifest.append({"url":u,"type":_clf_pdf(u,it.get("title","")),"hash":sha(u)[:32],"domain":urlparse(u).netloc,"text":it.get("title","")[:80],"provider":"gcse","auth_score":_dom_score(urlparse(u).netloc)})
        except:pass
    seen=set();unique=[]
    for p in manifest:
        if p["url"] not in seen:seen.add(p["url"]);unique.append(p)
    manifest=unique[:30]
    auth_doms=[{"domain":d,"score":0.9,"count":sum(1 for m2 in manifest if m2.get("domain")==d)} for d in set(m2.get("domain","") for m2 in manifest)]
    mode="REAL" if manifest else "NO_SOURCES"
    log_ev("DA0_PDF_END",f"{len(manifest)}")
    return{"mode":mode,"source_manifest":manifest,"strategy_log":{"mode":mode,"pdfs":len(manifest)},"http_diag":diag,"authority_sources":sorted(auth_doms,key=lambda x:-x["score"])}
def _clf_pdf(url,text):
    low=(url+" "+text).lower()
    return"corrige" if any(k in low for k in["corrig","correction","answer","solution","bareme","mark scheme"]) else "sujet"
def _dom_score(dom):
    for kw,v in[(".gov",0.9),(".gouv",0.9),(".edu",0.8),(".ac.",0.7)]:
...
def mk_cep(src):
    ...
    for s in suj:
        ...
        for j,c in enumerate(cor):
            ...
            if sc>bs:bs,bj=sc,j
        if bj>=0 and bs>=1:c=cor[bj];used.add(bj);pairs.append({"sujet":s["url"],"corrige":c["url"],"sujet_hash":s["hash"],"corrige_hash":c["hash"],"pair_id":f"CEP_{len(pairs):04d}","pair_score":bs})
        else:unp.append({"url":s["url"],"reason":"no_match"})
    for j,c in enumerate(cor):
...
def real_da1(cep):
    ...
    for pair in cep.get("pairs",[]):
        ...
        dl.append({"pair_id":pid,"s_status":"OK" if ss in range(200,300) else f"FAIL_{ss}","c_status":"OK" if cs in range(200,300) else f"FAIL_{cs}"})
    ...
    log_ev("DA1_END",f"{len(dl)}p {roc}ocr")
    return{"dl_log":dl,"texts":texts,"text_mode":"OCR_REAL" if roc>0 else "OCR_NONE","real_ocr_count":roc}
# ═══ B6: ATOMS ═══
def _split_q(text):
    if not text:return[]
    for pat in[r'(?=(?:Sujet|Exercice|EXERCICE|Exercise|QUESTION|Question|Partie|PARTIE|Part)\s*\d+)',r'(?=\d+[\.\)]\s+[A-Z])']:
...
def _pts(text):
    for pat in[r'(\d+)\s*(?:points?|pts?|marks?)',r'\((\d+)\s*(?:pt|mk|point)',r'[Cc]oefficient\s*:?\s*(\d+)']:
...
def extract_atoms(texts):
    ...
    return atoms
# ═══ B7: QC (schema-based) ═══
QC_SCHEMA={"qi_text_min":40,"rqi_text_min":40,"points_required_unless_long":200,"placeholder_patterns":["[OCR_PENDING","[DL_FAIL","[PENDING","[HARVEST"]}
def calc_qc(atoms):
    qc=[]
    for a in atoms:
        qi,rqi=a["Qi"],a["RQi"]
        checks={"qi_len":len(qi["text"])>=QC_SCHEMA["qi_text_min"],
                "rqi_len":len(rqi["text"])>=QC_SCHEMA["rqi_text_min"],
                "points_or_long":(qi.get("points") is not None and qi["points"]>0) or len(qi["text"])>=QC_SCHEMA["points_required_unless_long"],
                "qi_no_placeholder":not any(k in qi["text"] for k in QC_SCHEMA["placeholder_patterns"]),
                "rqi_no_placeholder":not any(k in rqi["text"] for k in QC_SCHEMA["placeholder_patterns"])}
    ...
    return qc
# ═══ B8: FRT / ARI / TRIGGERS — deterministic, zero random ═══
...
def calc_ari(atoms,qc):
    """ARI: deterministic, ZERO random. Uses only text lengths + points ratios."""
    ...
    for a in atoms:
        ...
        # Deterministic difficulty: ratio of points * response complexity
        diff=round(min((pts/total)*min(len(rqi["text"])/max(len(qi["text"]),1),3),0.99),3)
        # Deterministic discrimination: text density proxy
...
def calc_triggers(ari,frt,f1,f2):
    ...
    return trig
# ═══ B9: F1/F2 — TEST_CLEAR mode ═══
def compute_f1_clear(atoms,frt,qc):
    """F1 in TEST_CLEAR mode. Not sealed. Not IP. For pipeline validation only."""
    ...
    # Weighted average of normalized exam scores
    scores=[]
    ...
    return{"score":f1_score,"status":"OK","formula_mode":"TEST_CLEAR","formula_version":"TEST_V0",
           ...
           "exam_scores":{k:v for k,v in exam_scores.items()},"n_exams":len(exam_scores),"n_items":len(valid_atoms)}
def compute_f2_clear(f1,ari,triggers):
    """F2 in TEST_CLEAR mode."""
    ...
    return{"score":f2_score,"status":"OK","formula_mode":"TEST_CLEAR","formula_version":"TEST_V0",
           ...
           "avg_difficulty":round(avg_diff,4),"avg_discrimination":round(avg_disc,4),"trigger_penalty":round(trig_penalty,4)}
def _f_empty(tag):
    return{"score":0,"status":"NO_DATA","formula_mode":"TEST_CLEAR","formula_version":"TEST_V0",
           "approved":False,"source":"R&D","note":"No valid data to compute."}
# ═══ GATES + PIPELINE + UI ═══
def chk_branch():
...
def run_gates(cap,vsp,src,cep,da1,atoms,frt,qc,f1,f2,ari,trig):
    ...
    G("GATE_TRIGGERS",True,"always_pass")  # triggers can be empty
    # F1/F2 gates per GPT instructions
    ...
    G("GATE_FORMULA_MODE",f1.get("formula_mode")==expected_mode and f2.get("formula_mode")==expected_mode,f"expect={expected_mode} got={f1.get('formula_mode','')}")
    G("GATE_DETERMINISM",True,"deferred")  # set after det check
    return{"gates":g,"global_verdict":"PASS" if all(x["verdict"]=="PASS" for x in g) else "FAIL"}
def det_check(fn,iso2,rid,n=3):
    """REPLAY: reread sealed artifacts, verify identical hashes."""
...
def run_pipeline(iso2,_replay=False,_sealed_src=None):
        ...
        if mf.exists():
            ...
            for fname,info in manifest.get("files",{}).items():
                ...
                if actual!=info.get("sha256",""):
                    f1={"score":0,"status":"A2_HASH_MISMATCH","formula_mode":"A2_PACK"};f2=f1;trig0=[];break
            else:
            ...
            f1={"score":0,"status":"A2_MISSING","formula_mode":"FAIL"};f2=f1;trig0=[]
    ...
    # Write artifacts
    al=[("AR0",ar0),("AuthorityProof",{"selected":ar0.get("selected"),"candidates":ar0.get("candidates",[])[:20],"proof_summary":[c for c in ar0.get("candidates",[]) if c.get("score",0)>=0.55][:5]}),
    ...
    # Update GATE_DETERMINISM
    for g in gr["gates"]:
        if g["gate"]=="GATE_DETERMINISM":g["verdict"]="PASS" if det["pass"] else "FAIL";g["evidence"]=f"{'PASS' if det['pass'] else 'FAIL'} {det['hashes'][:2]}"
...
def main():
    st.set_page_config(page_title="SMAXIA GTE CORE v14.3.4",layout="wide")
    st.title(f"SMAXIA GTE CORE v14.3.4 — {EXEC_MODE} MODE")
    ...
    with T[1]:
        ...
        if not p:nd()
            ...
            if ar["status"]=="OK":
                ...
                for c in ar.get("candidates",[])[:15]:st.write(f"{'✓' if c.get('accessible') else '✗'} {c['domain']} s={c['score']} {c.get('proof','')} {c.get('reasons',[][:3])}")
    ...
    with T[3]:
        ...
        if not p:nd()
            ...
            for s in p["sources"]["source_manifest"][:20]:st.write(f"`{s['type']}` [{s.get('provider','')}] auth:{s.get('auth_score',0)} {s['url'][:80]}")
    with T[4]:
        p=st.session_state.pipeline
        if not p:nd()
        else:
            st.header("CEP");st.metric("Pairs",p["cep"]["total_pairs"])
            for pr in p["cep"]["pairs"]:st.write(f"**{pr['pair_id']}** score:{pr['pair_score']}")
            for u in p["cep"]["unpaired"]:st.write(f"✗ {u['reason']} {u['url'][:60]}")
    with T[5]:
        p=st.session_state.pipeline
        if not p:nd()
        else:
            da1=p["da1"];st.header(f"Text/OCR — {da1['text_mode']}");st.write(f"Real OCR: {da1['real_ocr_count']}")
            for pid in sorted(da1["texts"]):
                ...
                with st.expander(f"{pid} S:{len(t.get('sujet_text',''))}c C:{len(t.get('corrige_text',''))}c"):
                    ...
                    if t.get("corrige_text"):st.text_area("Corrigé",t["corrige_text"][:5000],height=150,key=f"c_{pid}",disabled=True)
    with T[6]:
        p=st.session_state.pipeline
        if not p:nd()
        else:
    ...
    with T[10]:
        ...
        if not p:nd()
            ...
            with c1:
                ...
                st.write(f"Mode: **{p['f1'].get('formula_mode','')}** v{p['f1'].get('formula_version','')}")
                st.write(f"Approved: {p['f1'].get('approved',False)} | Source: {p['f1'].get('source','')}")
            ...
            with c2:
                ...
                st.write(f"Mode: **{p['f2'].get('formula_mode','')}**")
                st.caption(p['f2'].get('note',''))
            st.subheader("ARI")
            for a in p["ari"]:st.write(f"**{a['atom_id']}** diff:{a['difficulty']} disc:{a['discrimination']}")
            st.subheader("Triggers")
            for t in p["triggers"]:st.write(f"**{t['trigger']}** {t.get('atom_id',t.get('theme',''))}={t['value']}")
    with T[11]:
        p=st.session_state.pipeline
        if not p:nd()
        else:
            ...
            st.subheader("Determinism");st.json(p["determinism"])
